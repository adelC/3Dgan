...
from networks.pgan.loss_utils import ecal_sum, \
    ecal_angle  # physics functions used for training the discriminator (should be in conditional lambda layer)
...
def forward_generator(generator,
    ...
    if latent_design == 'concatenate energy and angle':  # 254 random + energy + angle
        ...
        z = tf.math.multiply(energy_input_tensor,
                             z)  # try multiplying the latent space by the energy...tf.math.multiply should take care of broadcasting
    elif latent_design == 'multiply by both energy and angle':
        z = tf.random.normal(shape=[z_batch_size, latent_dim])
        z = tf.math.multiply(energy_input_tensor,
                             z)  # try multiplying the latent space by the energy...tf.math.multiply should take care of broadcasting
        z = tf.math.multiply(ang_input_tensor,
                             z)  # try multiplying the latent space by the angle...tf.math.multiply should take care of broadcasting
    ...
    disc_fake_g, fake_ecal, fake_ang = discriminator(gen_sample, alpha, phase, num_phases, base_shape, base_dim,
                                                     latent_dim,
                                                     activation=activation, param=leakiness, size=network_size,
                                                     is_reuse=is_reuse)
    if loss_fn == 'wgan':
...
def forward_simultaneous(generator,
    ...
    disc_fake_d, fake_ecal, fake_ang = discriminator(tf.stop_gradient(gen_sample_noisy), alpha, phase, num_phases,
                                                     base_shape, base_dim, latent_dim, activation=activation,
                                                     param=leakiness,
    ...
    disc_fake_g, fake_ecal_g, fake_ang_g = discriminator(gen_sample, alpha, phase, num_phases, base_shape, base_dim,
                                                         latent_dim,
                                                         activation=activation, param=leakiness, size=network_size,
                                                         is_reuse=True, conditioning=conditioning)
    if loss_fn == 'wgan':
        gradient_penalty = (slopes - 1) ** 2
        gp_loss = gp_weight * gradient_penalty
        disc_loss = disc_fake_d - disc_real
    ...
    #################################################anglepgan#ach#emmac START
        ...
        ang_loss = tf.reduce_mean(ang_loss)
        ecal_loss = tf.reduce_mean(ecal_loss)
